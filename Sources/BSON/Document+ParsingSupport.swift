...
extension Document {
    ...
    internal func index(recursive keys: [IndexKey]? = nil, lookingFor matcher: [IndexKey]?, levels: Int? = nil) -> ElementMetadata? {
        ...
        if let keys = keys {
            ...
            self.searchTree.recursivelyIndexed = self.searchTree.storage.values.reduce(true) {_, indexTrieNode in
                return indexTrieNode.recursivelyIndexed
                //                $0.1.recursivelyIndexed && $0.0
            }
        }
